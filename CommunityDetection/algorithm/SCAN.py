...
class SCAN():
    ...
    def get_hubs_outliers(self, communities):
        ...
        hubs = []
        outliers = []
#        print node_community  #已分类的节点对应的社区的dict
#        print other_nodes   #不属于任何社区的节点
        
        for node in other_nodes:
            neighbors = self._G.neighbors(node)   #每个节点的令居列表
            neighbors =  list(set(neighbors).intersection(set(node_community.keys()))) #筛选出每个节点的令居列表中已分类的节点
            #print neighbors
