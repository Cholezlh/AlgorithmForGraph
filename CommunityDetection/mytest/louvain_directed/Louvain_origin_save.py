...
def loadData(filePath):
    ...
    for line in f.readlines():
        ...
        for i in xrange(2):
            if lines[i] not in vector_dict.keys():
...
def modularity(vector_dict, edge_dict):
    ...
    for i in community_dict.keys():  #遍历每个社团
        ...
        Q += ((sum_in / (2*m)) - (sum_tot/(2*m))*(sum_tot/(2*m)))
    return Q
...
if __name__ == "__main__":
    vector_dict, edge_dict=loadData("staticInOut4_1.csv")
    #print vector_dict   #{'1': True, '0': True, '3': True, '2': True,...}
    #print edge_dict     #{'1': ['2:1', '4:1', '7:1'], '0': ['2:1', '3:1', '4:1', '5:1'],...}
    community_dict = fast_unfolding(vector_dict, edge_dict)
    with open("vertice_directedLouvain.csv",'w') as FILEOUT:
        for cid in community_dict.keys():
            vlist = community_dict[cid]
            for v in vlist:
                print >>FILEOUT,str(v)+","+str(cid)
